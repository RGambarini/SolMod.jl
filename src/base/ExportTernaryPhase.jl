...
"""
function exportTernaryPhase(exp, outputfile::String; 
    ...
    for t in range(1, length(temperatures))
      ...
      df = DataFrame(x_1 = exp[temperatures[t]][1:end, 1], 
      x_2 = exp[temperatures[t]][1:end, 2], x_3 = exp[temperatures[t]][1:end, 3])
  
      XLSX.openxlsx(outputfile, mode="rw") do xf
          numb_of_sheets = XLSX.sheetcount(xf)
          XLSX.addsheet!(xf, temperatures[t])
