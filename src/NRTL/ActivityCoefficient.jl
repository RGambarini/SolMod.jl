...
𝜏 defines the temperature dependency in the equation. The parameter g is obtained from experimental data, where g_ij is the interaction between two components in the solution while g_jj can be chosen freely as a reference state. R is defined as the ideal gas constant. 𝜏 can then be used in the definition for G, which also contains the nonrandomness parameter ⍺. This parameter has no true physical meaning, and can be chosen at a value close to 0.3 in most literature
