function NRTL_ternaryPhase(params::Dict, solvent::String, Tx::Vector; x_step::Float64 = 0.001, 
  pp = true, round1::Int64 = 4, round2::Int64 = 4, round3::Int64 = 3, R = 8.314)
...
# 3. Tx = Vector that includes the temperatures used for isotherm of the ternary 
# phase diagram
...
# solubility points beyond the eutectic points
  ...
  for t in Tx
    ...
    function ei(xi) NRTLei(xi, t) end
    function ej(xi) NRTLej(xi, t) end
    function r(xi, xj) NRTLr(xi, xj, t) end
    ...
    if pp == true
      ...
      println("Eutectic points of isotherm "*string(t)*"K are at composition: ")
      println("")
      ...
      model_1 = vcat(modelx, modely)

      rem = []

      for i in range(1, length(model_1[1:end, 1]))
          if model_1[i, 3] <= min[3]
              append!(rem, i)
          end
      end
      
      model[t] = model_1[setdiff(1:end, rem), :]
    
    else

      model[t] = vcat(modelv, modelr)
